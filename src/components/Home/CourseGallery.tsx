...
const CoursesSection = () => {
  const icons = [
    <IoCalculator
      size={80}
      className="group-hover:text-orange-400 text-orange-200 group-hover:scale-105"
    />,
    <AiOutlineCompass
      size={80}
      className="group-hover:text-orange-400 text-orange-200 group-hover:scale-105"
...
function CourseCard({ index, item, icon }: CourseCardProps) {
  return (
    <Link
      ...
      className="group relative border bg-white bg-blend-hue w-full gap-4 flex flex-col p-4 sm:p-6 lg:p-6 py-10   max-w-[300px] justify-center items-center hover:shadow-[4px_4px_25px_-10px_rgba(251,146,60,0.5)] rounded-xl overflow-clip "
    >
      {/* <img
        alt={item.title}
